...
// Get the actual projects directory (handles nested 'projects' folder)
function getActualProjectsDir() {
  const projectsSubfolder = path.join(PROJECTS_DIR, 'projects');
  if (fs.existsSync(projectsSubfolder) && fs.statSync(projectsSubfolder).isDirectory()) {
    return projectsSubfolder;
  }
  return PROJECTS_DIR;
}
...
function getProjects() {
  try {
    let searchDir = PROJECTS_DIR;
    
    // If the configured directory has a 'projects' subfolder, use that instead
    const projectsSubfolder = path.join(PROJECTS_DIR, 'projects');
    if (fs.existsSync(projectsSubfolder) && fs.statSync(projectsSubfolder).isDirectory()) {
      searchDir = projectsSubfolder;
    }
    
    const entries = fs.readdirSync(searchDir, { withFileTypes: true });
    // Filter out non-project directories and files
    const excludedNames = ['node_modules', '.git', '.gitignore', '.DS_Store', '.vscode', '.idea', 'package.json', 'package-lock.json', 'README.md', 'LICENSE', 'projects'];
    
    return entries
      // Only include directories (folders), never files
      .filter(entry => {
        ...
        // Exclude all hidden directories (starting with .)
        if (name.startsWith('.')) {
          return false;
        }
        return true;
...
function getProjectVariants(projectName) {
  const actualProjectsDir = getActualProjectsDir();
  const projectPath = path.join(actualProjectsDir, projectName);
  try {
...
function processHtmlImports(jsContent, variantPath) {
  ...
  for (let i = matches.length - 1; i >= 0; i--) {
    const [fullMatch, varName, relativePath, htmlFileName] = matches[i];
    
    // Skip if this variable is already declared (prevents duplicate processing)
    // Check for const, let, or var declarations
    const varDeclarationRegex = new RegExp(`(?:const|let|var)\\s+${varName}\\s*[=;]`, 'g');
    if (varDeclarationRegex.test(processedContent)) {
      // Variable already exists, just remove the import statement
      processedContent = processedContent.replace(fullMatch, '');
      console.log(`[ELI] Skipping duplicate HTML import: ${htmlFileName} -> ${varName} (already declared)`);
      continue;
    }
    
    // Escape the fullMatch for regex replacement
    const escapedFullMatch = fullMatch.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    
    // Check if this import statement still exists in the content
    // (it might have been removed by a previous iteration)
    if (!processedContent.includes(fullMatch)) {
      continue;
    }
    ...
    try {
      if (fs.existsSync(htmlPath)) {
        ...
        // Use the original fullMatch for string replacement (not the escaped regex version)
...
function processJsImports(jsContent, variantPath, projectPath, processingUtils = false) {
  ...
  while ((match = importRegex.exec(originalContent)) !== null) {
    ...
    if (importPath === '@eli/utils' || importPath === '@eli/utils.js') {
      ...
      // Check for node_modules/@briannorman9/eli-utils in project directory and parent directories
      let searchPath = projectPath;
      while (searchPath && searchPath !== path.dirname(searchPath)) {
        const nodeModulesPath = path.join(searchPath, 'node_modules', '@briannorman9', 'eli-utils');
        ...
        if (fs.existsSync(packageJsonPath) && fs.existsSync(indexJsPath)) {
          utilsPath = indexJsPath;
          console.log(`[ELI] Found @briannorman9/eli-utils in node_modules: ${utilsPath}`);
          break;
        }
      ...
      try {
        if (utilsPath && fs.existsSync(utilsPath)) {
          ...
          // Process any imports in utils (though it shouldn't have any)
          ...
          console.warn(`[ELI] @eli/utils not found. Install it via: npm install @briannorman9/eli-utils`);
          processedContent = processedContent.replace(fullMatch, `const ${varName} = {};`);
        }
      } catch (error) {
    ...
    // Skip HTML and SCSS imports - they should already be processed by processHtmlImports and processScssImports
    if (importPath.endsWith('.html') || importPath.endsWith('.scss') || importPath.endsWith('.css')) {
      continue;
    }
    
    // Skip if not a .js file (might be other file types)
...
function getVariantScript(projectName, variantName) {
  const actualProjectsDir = getActualProjectsDir();
  const variantPath = path.join(actualProjectsDir, projectName, variantName);
  try {
...
async function saveMinifiedScript(projectName, variantName, minifiedCode, jsFileName = null, silent = false) {
  try {
    const actualProjectsDir = getActualProjectsDir();
    const variantPath = path.join(actualProjectsDir, projectName, variantName);
    ...
    if (jsFileName) {
      // Use the JS file name (e.g., v2.js -> v2.min.js)
      minifiedFileName = jsFileName.replace(/\.js$/, '.min.js');
    } else {
      // Fallback to variant name if JS filename not provided
      minifiedFileName = `${variantName}.min.js`;
    }
    
    const minifiedPath = path.join(variantPath, minifiedFileName);
    fs.writeFileSync(minifiedPath, minifiedCode, 'utf8');
    if (!silent) {
      console.log(`[ELI] Saved minified script: ${projectName}/${variantName}/${minifiedFileName}`);
    }
    return minifiedPath;
  } catch (error) {
    ...
    (async () => {
      try {
        ...
        if (scriptData && scriptData.content) {
          ...
          if (shouldSave) {
            await saveMinifiedScript(projectName, variantName, minifiedCode, scriptData.filename);
          }
...
async function autoMinifyOnSave(filePath) {
  try {
    // Resolve to absolute paths to handle any path format
    const resolvedFilePath = path.resolve(filePath);
    const actualProjectsDir = getActualProjectsDir();
    const resolvedProjectsDir = path.resolve(actualProjectsDir);
    
    // Only process files in variant folders or shared.js at project root
    const relativePath = path.relative(resolvedProjectsDir, resolvedFilePath);
    
    // Skip if file is outside actual projects dir (path.relative returns path starting with ..)
...
async function minifyVariant(projectName, variantName, silent = true) {
  try {
    ...
    if (scriptData && scriptData.content) {
      const minifiedCode = await minifyScript(scriptData.content);
      await saveMinifiedScript(projectName, variantName, minifiedCode, scriptData.filename, silent);
      if (!silent) {
...
function setupFileWatcher() {
  ...
  if (!fs.existsSync(actualProjectsDir)) {
    console.warn(`[ELI] Projects directory does not exist: ${actualProjectsDir}. File watcher not started.`);
    console.warn(`[ELI] Configure the projects directory via the Chrome extension or set PROJECTS_DIR environment variable.`);
    return;
  }

  try {
    fileWatcher = chokidar.watch(actualProjectsDir, {
    ...
    console.log(`[ELI] File watcher active for: ${actualProjectsDir}`);
  } catch (error) {
...
server.listen(PORT, () => {
  ...
  const actualProjectsDir = getActualProjectsDir();
  if (actualProjectsDir !== PROJECTS_DIR) {
    console.log(`(Using nested projects folder: ${actualProjectsDir})`);
  }
  console.log(`\nAvailable projects: ${getProjects().join(', ') || 'none'}`);
  console.log('\nServer ready! Make changes to your project scripts and reload the page.');
  
  // Start file watcher for auto-minification (only if directory exists)
  // If directory doesn't exist, the watcher will be set up when the directory is configured via API
  if (fs.existsSync(actualProjectsDir)) {
    setupFileWatcher();
  } else {
    console.log(`[ELI] Projects directory does not exist yet. File watcher will start when directory is configured.`);
  }
});
