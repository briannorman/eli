...
function processJsImports(jsContent, variantPath, projectPath) {
  // Match: import variableName from './filename.js' or '../filename.js'
  ...
  while ((match = importRegex.exec(originalContent)) !== null) {
    const [fullMatch, varName, jsFilePath] = match;
    
    // Resolve the path relative to the variant folder
    let jsPath;
    if (jsFilePath.startsWith('./') || jsFilePath.startsWith('../')) {
      // Relative path - resolve from variant folder
      jsPath = path.resolve(variantPath, jsFilePath);
    } else {
      // Absolute path from project root
      jsPath = path.join(projectPath, jsFilePath);
    }
    
    // Security: ensure the resolved path is within the project directory
    const normalizedPath = path.normalize(jsPath);
    if (!normalizedPath.startsWith(projectPath)) {
      console.warn(`[ELI] JS import path outside project: ${jsFilePath}`);
      processedContent = processedContent.replace(fullMatch, '');
      continue;
    }
    ...
    try {
      if (fs.existsSync(jsPath)) {
        ...
        // Check if the file exports a default value
        const exportDefaultMatch = jsFileContent.match(/export\s+default\s+(.+);?\s*$/);
        
        let replacement;
        if (exportDefaultMatch) {
          // File exports a value - use it
          const exportedValue = exportDefaultMatch[1].trim();
          replacement = `const ${varName} = ${exportedValue};`;
        } else {
        ...
        processedContent = processedContent.replace(fullMatch, replacement);
        console.log(`[ELI] Processed JS import: ${jsFilePath} -> ${varName}`);
      } else {
      ...
      console.error(`[ELI] Error processing JS import ${jsFilePath}:`, error);
      processedContent = processedContent.replace(fullMatch, `const ${varName} = undefined;`);
    }
  }
...
function getVariantScript(projectName, variantName) {
  ...
  try {
    ...
    console.error(`Error reading variant ${variantName} for ${projectName}:`, error);
  }
  return null;
}
